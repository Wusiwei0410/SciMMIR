...
class LLaMA_adapter(nn.Module):
    ...
    def _get_text_features(self,  prompts,max_gen_len: int = 256,max_length=256):
        bsz = len(prompts)
        ...
        # total_len = params.max_seq_len  #max_length=256
        total_len=max_length

        tokens = torch.full((bsz, total_len), self.tokenizer.pad_id).cuda().long()
        
        tokens= torch.tensor(prompts).cuda().long()
